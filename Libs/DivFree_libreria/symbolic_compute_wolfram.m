...
function symbolic_compute_wolfram(SaveDire,sym_vars,fbr,OperadoresY,OperadoresX)
...
Phi_div  =  jacobian(gradient(fbr,[x1,y1]),[x1 , y1]) ...
    - [laplacian(fbr,[x1,y1]), 0 ; 0 , laplacian(fbr,[x1,y1])] ;
...
for j= 1:m
    for i= 1:n
        ...
        commando = ['wolframscript -code ','"FullSimplify[Simplify[', char(fbraux),']]"' ];
        %commando = ['wolframscript -code ','"Parallelize[FullSimplify[Simplify[', char(fbraux),']]]"' ];
        [status,cmdout] = system(commando);
        fbraux =str2sym(regexprep(cmdout,'\n+',''));
