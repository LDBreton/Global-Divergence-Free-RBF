...
Loaddir = 'ConstRFBRsaveFiles/';
[fbrGram,fbrAnzatz] = LoadFbrFiles(Loaddir,4,6);
Savetable = 'NewTables/';
mkdir(Savetable);

%creando las normales
Symbolic_normal_angular(Loaddir)
class_t = 'mp'; 
...
Avar_dt = [mp('0.05'),mp('0.01'),mp('0.001')];
Apoint = [20,25,30,40,50,80];

for lk=1:length(Avar_muu)
    fileID = fopen([Savetable,'Errores_muu_',num2str(double(Avar_muu(lk))),'_slip.txt'],'w');
    for k=1:length(Apoint)
        for j=1:length(Avar_dt)
            [P_ni,P_f] = Mesh_gen_N(Apoint(k),0.5);
            var_muu = Avar_muu(lk);
            var_dt = Avar_dt(j);
            var_c = mp('2');
